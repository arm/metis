...
Once the service responds, run `uv run metis --interactive --codebase-path <path>` (or `metis --interactive` inside your virtual environment) and use the usual `index`, `review_code`, `review_dir` or `review_file` commands. Metis will route model requests through the OpenAI Responses API and embedding requests through the OpenAI-compatible embeddings API.
