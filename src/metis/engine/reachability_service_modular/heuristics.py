...
"""Lightweight source/sink heuristics used while building the full graph."""
...
def sink_type_for_calls(calls: list[str], text: str = "") -> str:
    ...
    if lower_calls & {"free", "kfree", "vfree"} or re.search(
        r"\b(?:release|destroy|cleanup)\b", lowered_text
    ):
...
def is_sink_function(
    ...
    if SINK_NAME_RE.search(name or ""):
        return (
            True,
            sink_type_for_calls(calls, text),
            "function name matches security-sensitive state/resource pattern",
        )
    return False, "", ""
