...
class PartialReachabilityFileService:
    ...
    def review_file(
        ...
        *,
        ...
        target_nodes, target_globals = self._extract_target(
            abs_target, rel_target, max_workers, progress_callback, cache
        )
        ...
        context_builder.expand_companions(
            context, index, progress_callback=progress_callback
        )
        ...
        detector_result = PartialCandidateDetector(
            self._config.codebase_path, cache
        ).detect(index, rel_target, context.target_nodes, context)
            ...
            progress_callback(
                {
                    ...
                    "allocation_arithmetic": len(
                        detector_result.allocation_arithmetic_notes
                    ),
                    ...
                    "alias_extent_mismatch": len(
                        detector_result.alias_extent_mismatch_notes
                    ),
                    ...
                    "region_replace_erase": len(
                        detector_result.region_replace_erase_notes
                    ),
                    "metadata_type_confusion": len(
                    ...
                    "zero_count_underflow": len(
                        detector_result.zero_count_underflow_notes
                    ),
                    "owner_liveness_allocation": len(
                        detector_result.owner_liveness_notes
                    ),
                    "user_buffer_permission": len(
                        detector_result.user_buffer_permission_notes
                    ),
                    "zone_shrink_validation": len(detector_result.zone_shrink_notes),
                    "success_path_cleanup": len(
                        detector_result.success_path_cleanup_notes
                    ),
                    ...
                    "cross_file_lock_cycles": len(
                        detector_result.cross_file_lock_notes
                    ),
                    "named_lock_inversion": len(
                        detector_result.named_lock_inversion_notes
                    ),
    ...
    def _ensure_symbol_index(self, *, progress_callback=None):
        with self._index_lock:
            ...
            if progress_callback:
                progress_callback(
                    {"event": "partial_symbol_index_start", "files": len(files)}
                )
            self._symbol_index = SymbolIndexBuilder().build(
                files, self._config.codebase_path
            )
            if progress_callback:
                progress_callback(
                    {
                        ...
                        "definitions": sum(
                            len(v) for v in self._symbol_index.definitions.values()
                        ),
                        "callsites": sum(
                            len(v) for v in self._symbol_index.callsites.values()
                        ),
    ...
    def _extract_target(
        self, abs_target, rel_target, max_workers, progress_callback, cache
    ):
        if progress_callback:
            progress_callback(
                {"event": "partial_target_extract_start", "file": rel_target}
            )
        defs = (
            _symbols_for_file(self._symbol_index, rel_target)
            if self._symbol_index is not None
            else []
        )
        nodes = [
    ...
    def _merge_detector_context(
        self, context: PartialReviewContext, detector_result: PartialDetectorResult
    ):
        if detector_result.globals:
            context.globals = self._merge_globals(
