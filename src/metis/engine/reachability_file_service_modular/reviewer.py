...
class TargetedFileReviewer:
    ...
    def review(
        ...
        partial_graph: ReachabilityGraph,
        *,
        ...
        with ThreadPoolExecutor(
            max_workers=max(1, min(max_workers, len(passes)))
        ) as ex:
            futs = {
                submit_with_current_context(
                    ex, self._run_pass, context, partial_graph, item, detector_result
                ): item[0]
                for item in passes
            }
            for fut in as_completed(futs):
                ...
                try:
                    ...
                    logger.warning(
                        "Partial review pass failed for %s: %s", pass_name, exc
                    )
        ...
        if progress_callback:
            progress_callback(
                {"event": "partial_review_raw_done", "raw_findings": len(findings)}
            )
        return findings

    def _build_passes(self, context, graph, detector_result):
        detector_nodes = detector_result.nodes or []
        copy_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.copy_contract_notes
        )
        cleanup_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.cleanup_symmetry_notes
        )
        accounting_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.accounting_drift_notes
        )
        arithmetic_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.arithmetic_chain_notes
        )
        resource_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.resource_binding_notes
        )
        policy_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.policy_gate_notes
        )
        resource_validation_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.resource_validation_notes
        )
        cleanup_ledger_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.cleanup_ledger_notes
        )
        async_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.async_order_notes
        )
        size_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.size_propagation_notes
        )
        tracker_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.stale_tracker_notes
        )
        type_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.metadata_type_confusion_notes
        )
        pm_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.pm_sequence_notes
        )
        secondary_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.secondary_omission_notes
        )
        protected_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.protected_mmu_notes, cap=40
        )
        mmu_recovery_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.mmu_recovery_notes, cap=32
        )
        sentinel_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.sentinel_misuse_notes
        )
        suspend_cleanup_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.suspend_cleanup_ledger_notes, cap=40
        )
        suspend_size_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.suspend_size_sink_notes, cap=40
        )
        fault_clear_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.fault_clear_order_notes, cap=40
        )
        pm_callback_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.pm_callback_order_notes, cap=32
        )
        region_replace_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.region_replace_erase_notes, cap=32
        )
        imported_mapping_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.imported_mapping_policy_notes, cap=40
        )
        alias_extent_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.alias_extent_mismatch_notes, cap=40
        )
        named_lock_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.named_lock_inversion_notes, cap=48
        )
        active_singleton_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.active_singleton_stale_notes, cap=40
        )
        zero_count_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.zero_count_underflow_notes, cap=24
        )
        owner_liveness_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.owner_liveness_notes, cap=32
        )
        user_buffer_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.user_buffer_permission_notes, cap=32
        )
        zone_shrink_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.zone_shrink_notes, cap=32
        )
        success_cleanup_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.success_path_cleanup_notes, cap=32
        )
        jit_lock_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.jit_lock_protocol_notes, cap=48
        )
        teardown_order_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.teardown_order_notes, cap=48
        )
        queue_publish_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.queue_publish_init_notes, cap=32
        )
        fd_reuse_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.fd_reuse_notes, cap=32
        )
        debugfs_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.debugfs_permission_notes, cap=32
        )
        lock_cycle_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.cross_file_lock_notes, cap=48
        )
        protocol_nodes = self._nodes_for_notes(
            detector_nodes, detector_result.protocol_notes, cap=48
        )
        passes = [
            ...
            (
                ...
                context.lifecycle_pair_nodes + context.callback_nodes,
            ),
            (
                "concurrency",
                context.target_nodes,
        ...
        if (
            detector_result.state_publication_notes
            or detector_result.disable_stale_notes
        ):
            passes.append(
                (
                    "state_publication",
        ...
        if detector_result.publish_rollback_notes:
            passes.append(
                (
                    "publish_rollback",
                    context.target_nodes,
                    context.lifecycle_pair_nodes + detector_nodes,
                )
            )
        ...
        if detector_result.cleanup_symmetry_notes:
            passes.append(
                (
                    "cleanup_symmetry",
                    context.target_nodes,
                    context.lifecycle_pair_nodes + cleanup_nodes,
                )
            )
        if detector_result.accounting_drift_notes:
            passes.append(
                (
                    "accounting_drift",
                    context.target_nodes,
                    context.shared_state_nodes + accounting_nodes,
                )
            )
        ...
        if detector_result.suspend_cleanup_ledger_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.companion_nodes + suspend_cleanup_nodes,
                )
            )
        ...
        if detector_result.arithmetic_chain_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.outbound_callees + arithmetic_nodes,
                )
            )
        if detector_result.size_propagation_notes:
            passes.append(
                (
                    "size_propagation",
                    context.target_nodes,
                    context.outbound_callees + context.companion_nodes + size_nodes,
                )
            )
        ...
        if detector_result.async_order_notes:
            passes.append(
                (
                    "async_event_order",
                    context.target_nodes,
                    context.callback_nodes + context.companion_nodes + async_nodes,
                )
            )
        ...
        if detector_result.stale_tracker_notes:
            passes.append(
                (
                    "stale_tracker_state",
                    context.target_nodes,
                    context.shared_state_nodes + tracker_nodes,
                )
            )
        ...
        if detector_result.pm_sequence_notes:
            passes.append(
                (
                    "pm_runtime_sequence",
                    context.target_nodes,
                    context.companion_nodes + pm_nodes,
                )
            )
        if detector_result.pm_callback_order_notes:
            passes.append(
                (
                    "pm_callback_order",
                    context.target_nodes,
                    context.companion_nodes + pm_callback_nodes,
                )
            )
        if detector_result.secondary_omission_notes:
            passes.append(
                ("secondary_element_omission", context.target_nodes, secondary_nodes)
            )
        if detector_result.zero_count_underflow_notes:
            passes.append(
                ("zero_count_underflow", context.target_nodes, zero_count_nodes)
            )
        if detector_result.owner_liveness_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.companion_nodes + owner_liveness_nodes,
                )
            )
        if detector_result.user_buffer_permission_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.companion_nodes + user_buffer_nodes,
                )
            )
        ...
        if detector_result.success_path_cleanup_notes:
            passes.append(
                ("success_path_cleanup", context.target_nodes, success_cleanup_nodes)
            )
        ...
        if detector_result.queue_publish_init_notes:
            passes.append(
                (
                    "queue_publish_init",
                    context.target_nodes,
                    context.shared_state_nodes + queue_publish_nodes,
                )
            )
        if detector_result.fd_reuse_notes:
            passes.append(
                (
                    "fd_reuse_race",
                    context.target_nodes,
                    context.companion_nodes + fd_reuse_nodes,
                )
            )
        ...
        if detector_result.mmu_recovery_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.outbound_callees + mmu_recovery_nodes,
                )
            )
        if detector_result.allocation_arithmetic_notes:
            passes.append(
                (
                    ...
                    context.target_nodes,
                    context.outbound_callees + detector_nodes,
                )
            )
        ...
        if detector_result.active_singleton_stale_notes:
            passes.append(
                (
                    ...
                    + context.lifecycle_pair_nodes
                    + context.callback_nodes
                    + active_singleton_nodes,
                )
            )
        if self._should_add_partial_exact_fallback(detector_result):
            fallback_nodes = (
                context.companion_nodes[:24]
                + context.shared_state_nodes[:24]
                + context.lifecycle_pair_nodes[:16]
                + context.outbound_callees[:16]
            )
            passes.append(
                ("partial_exact_fallback", context.target_nodes, fallback_nodes)
            )
        return passes

    def _should_add_partial_exact_fallback(
        self, detector_result: PartialDetectorResult
    ) -> bool:
        strong = (
    ...
    def _nodes_for_notes(
        self, nodes: list[FunctionNode], notes: list[str], *, cap: int = 32
    ) -> list[FunctionNode]:
            ...
            return sorted(
                ...
                key=lambda node: min(
                    (
                        ...
                        for key in (
                            node.unique_name,
                            f"{node.file_path}::{node.name}",
                            node.name,
                        )
                        if key and text.find(key) >= 0
                    ),
                    default=len(text),
                ),
            )
    ...
    def _run_pass(self, context, graph, pass_item, detector_result):
        ...
        target_code = self._build_code(
            target_nodes, per_fn_chars=4500, max_total_chars=42000
        )
        context_per_fn, context_total = self._context_code_budget(pass_name)
        context_code = self._build_code(
            context_nodes, per_fn_chars=context_per_fn, max_total_chars=context_total
        )
        if not target_code:
            return []
        ...
        if self._reasoning_effort and str(self._reasoning_effort).lower() not in {
            "none",
            "off",
            "false",
            "default",
            ...
            .invoke(
                {
                    ...
                    "scope_rule": self._scope_rule_for_pass(
                        pass_name, context.target_file
                    ),
                    "paths_section": self._paths_section(
                        context.candidate_paths, graph
                    ),
                    "candidate_notes": self._candidate_notes_for_pass(
                        pass_name, detector_result
                    ),
        ...
        return self._parse_findings(
            raw, context, graph, analysis_type=f"partial_{pass_name}"
        )
    ...
    def _candidate_notes_for_pass(
        self, pass_name: str, detector_result: PartialDetectorResult
    ) -> str:
        mapping = {
            "state_publication": (
            ...
            "publish_rollback": (
                ("PUBLISH_ROLLBACK", detector_result.publish_rollback_notes),
            ),
            "allocation_arithmetic": (
                ("ALLOCATION_ARITHMETIC", detector_result.allocation_arithmetic_notes),
            ),
            "copy_contract": (("COPY_CONTRACT", detector_result.copy_contract_notes),),
            "cleanup_symmetry": (
                ("CLEANUP_SYMMETRY", detector_result.cleanup_symmetry_notes),
            ),
            "accounting_drift": (
                ("ACCOUNTING_DRIFT", detector_result.accounting_drift_notes),
            ),
            "cleanup_ledger": (
                ("CLEANUP_LEDGER", detector_result.cleanup_ledger_notes),
            ),
            "suspend_cleanup_ledger": (
                (
                    "SUSPEND_CLEANUP_LEDGER",
                    detector_result.suspend_cleanup_ledger_notes,
                ),
            ...
            "resource_validation_order": (
                (
                    "RESOURCE_VALIDATION_ORDER",
                    detector_result.resource_validation_notes,
                ),
            ),
            "arithmetic_chain_mismatch": (
                ...
                (
                    "ALLOCATION_ARITHMETIC",
                    detector_result.allocation_arithmetic_notes[:12],
                ),
            ),
            "alias_extent_mismatch": (
                ...
                (
                    "ARITHMETIC_CHAIN_MISMATCH",
                    detector_result.arithmetic_chain_notes[:8],
                ),
            ),
            "size_propagation": (
                ...
                (
                    "ARITHMETIC_CHAIN_MISMATCH",
                    detector_result.arithmetic_chain_notes[:12],
                ),
            ),
            "resource_binding_order": (
            ...
            "stale_tracker_state": (
                ("STALE_TRACKER_STATE", detector_result.stale_tracker_notes),
            ),
            "region_replace_erase": (
            ...
            "metadata_type_confusion": (
                (
                    "METADATA_TYPE_CONFUSION",
                    detector_result.metadata_type_confusion_notes,
                ),
            ),
            "pm_runtime_sequence": (
                ("PM_RUNTIME_SEQUENCE", detector_result.pm_sequence_notes),
            ),
            "pm_callback_order": (
            ...
            "secondary_element_omission": (
                (
                    "SECONDARY_ELEMENT_OMISSION",
                    detector_result.secondary_omission_notes,
                ),
            ),
            "zero_count_underflow": (
                ("ZERO_COUNT_UNDERFLOW", detector_result.zero_count_underflow_notes),
            ),
            "owner_liveness_allocation": (
                ("OWNER_LIVENESS_ALLOCATION", detector_result.owner_liveness_notes),
            ),
            "user_buffer_permission": (
                (
                    "USER_BUFFER_PERMISSION",
                    detector_result.user_buffer_permission_notes,
                ),
            ),
            "zone_shrink_validation": (
                ("ZONE_SHRINK_VALIDATION", detector_result.zone_shrink_notes),
            ),
            "success_path_cleanup": (
            ...
            "jit_lock_protocol": (
                ("JIT_LOCK_PROTOCOL", detector_result.jit_lock_protocol_notes),
            ),
            "teardown_order": (
                ("TEARDOWN_ORDER", detector_result.teardown_order_notes),
            ),
            "queue_publish_init": (
            ...
            "debugfs_permission": (
                ("DEBUGFS_PERMISSION", detector_result.debugfs_permission_notes),
            ),
            "protected_mmu_protocol": (
            ...
            "active_singleton_stale": (
                (
                    "ACTIVE_SINGLETON_STALE",
                    detector_result.active_singleton_stale_notes,
                ),
                ("PROTECTED_MMU_PROTOCOL", detector_result.protected_mmu_notes[:8]),
            ),
            "mmu_recovery_rollback": (
                ("MMU_RECOVERY_ROLLBACK", detector_result.mmu_recovery_notes),
            ),
            "sentinel_misuse": (
                ("SENTINEL_MISUSE", detector_result.sentinel_misuse_notes),
            ),
            "policy_gate_before_sink": (
                ("POLICY_GATE_BEFORE_SINK", detector_result.policy_gate_notes),
            ),
            "imported_mapping_policy": (
                (
                    "IMPORTED_MAPPING_POLICY",
                    detector_result.imported_mapping_policy_notes,
                ),
            ...
            "target_intra": (
                ...
                (
                    "FORMAT_OR_INFO",
                    (detector_result.format_notes + detector_result.info_leak_notes)[
                        :12
                    ],
                ),
            ),
    ...
    def _parse_findings(self, raw, context, graph, analysis_type):
        ...
        for entry in entries:
            ...
            fn = (
                self._lookup_node(
                    str(entry.get("function_name") or ""), by_name, by_unique
                )
                or target_default
            )
            related = self._lookup_node(
                str(entry.get("related_function") or ""), by_name, by_unique
            )
            if not fn:
                continue
            ...
            primary_function = (
                str(entry.get("primary_function") or "").strip() or fn.unique_name
            )
            ...
            results.append(
                VulnerabilityFinding(
                    id=os.urandom(8).hex(),
                    vulnerability_type=_normalise_partial_vuln_type(
                        entry.get("vulnerability_type") or "other"
                    ),
                    ...
                    sink_line=line,
                    path=(
                        [src.unique_name, fn.unique_name]
                        if related
                        else [fn.unique_name]
                    ),
