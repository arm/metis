...
class TriageGraph:
    ...
    def triage(self, request: TriageRequest) -> dict:
        ...
        model_status = out.get("decision_status", "")
        reason = out.get("decision_reason", "")
        if model_status not in {"valid", "invalid", "inconclusive"}:
            raise ValueError("Invalid triage status from model")
        if not reason:
            raise ValueError("Missing triage reason from model")
        evidence = out.get("decision_evidence") or []
        resolution_chain = out.get("decision_resolution_chain") or []
        unresolved_hops = out.get("decision_unresolved_hops") or []
        ...
        if status == "valid" and not evidence:
            raise ValueError("Valid triage decisions must include evidence citations")
