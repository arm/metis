...
def _collect_analyzer_sections(
    ...
    *,
    ...
    if not bool(getattr(evidence, "supported", False)):
        ...
        if unresolved:
            sections.append("[ANALYZER_UNRESOLVED]\n" + "\n".join(unresolved[:12]))
        return False, False, [], unresolved[:12]
    ...
    if citations:
        sections.append("[ANALYZER_CITATIONS]\n" + "\n".join(citations[:12]))
    if resolution_chain:
        sections.append(
            "[ANALYZER_RESOLUTION_CHAIN]\n" + "\n".join(resolution_chain[:12])
        )
    if flow_chain:
        sections.append("[ANALYZER_FLOW_CHAIN]\n" + "\n".join(flow_chain[:12]))
    if unresolved:
        sections.append("[ANALYZER_UNRESOLVED]\n" + "\n".join(unresolved[:12]))
    if fallback_targets:
        sections.append(
            "[ANALYZER_FALLBACK_TARGETS]\n" + "\n".join(fallback_targets[:12])
        )
    if extra_sections:
        sections.append("[ANALYZER_SECTIONS]\n" + "\n".join(extra_sections[:12]))

    has_citations = bool(citations)
    return True, has_citations, fallback_targets[:12], unresolved[:12]
...
def _collect_targeted_recovery_sections(
    ...
    *,
    ...
    targets = [t for t in fallback_targets if t][:3]
    for target in targets:
        ...
        for path in fallback_paths:
            ...
            if output is None:
                continue
            hits = _parse_grep_hits(output, max_hits=4)
            for hit_path, hit_line in hits[:2]:
                if len(sections) >= max_sections:
                    break
...
def _has_cross_boundary_unresolved_hops(unresolved_hops: list[str]) -> bool:
    ...
    for hop in unresolved_hops:
        ...
        if not text:
            continue
        for prefix in cross_boundary_prefixes:
...
def _merge_terms_with_fallback_targets(
    *,
    ...
    if not fallback_targets:
        return terms
    boosted_terms = [t for t in fallback_targets if t][:4]
...
def _finalize_evidence_pack_state(
    ...
    evidence_pack = "\n\n".join(sections)
    if len(evidence_pack) > 14000:
        evidence_pack = evidence_pack[:14000] + "\n...[truncated]"
