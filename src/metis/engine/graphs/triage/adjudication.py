...
_NON_CRITICAL_UNRESOLVED_PREFIXES = (
...
def _is_non_critical_unresolved_hop(hop: str) -> bool:
    text = str(hop or "").strip()
    if not text:
        return False
    for prefix in _NON_CRITICAL_UNRESOLVED_PREFIXES:
...
def has_critical_unresolved_hops(
    ...
    for hop in unresolved_hops:
        if not _is_non_critical_unresolved_hop(hop):
            return True
    return False
