...
class ReachabilityTriageService:
    def __init__(
        self,
        *,
        reachability_service: Any,
        ...
        self._reachability = reachability_service
        ...
        self._chat_model_kwargs = chat_model_kwargs
    ...
    def classifier(
        ...
        *,
    ...
    ) -> Callable[[Any, list[dict[str, Any]], object], dict | None]:
        unavailable = {self._normalize_path(path) for path in unavailable_files}
        options = ReachabilityReviewOptions.from_kwargs(
            self._settings,
            default_workers=self._max_workers,
        )
        graph = self._reachability.analysis_graph(
            options=options,
            codegraph=codegraph,
        )
        model_tools = (
            navigation_model_tools(
                navigation,
        ...
        runner = ReachabilityTriageRunner(
            self._llm_provider,
            options.confirmation_model or self._model,
            self._usage_runtime,
            self._codebase_path,
            options=options,
            chat_model_kwargs=self._chat_model_kwargs,
            model_tools=model_tools,
            max_tool_rounds=model_tool_max_rounds if model_tools else None,
        )

        def classify(
            finding: Any,
            threat_model_context: list[dict[str, Any]],
            _debug_callback: object,
        ) -> dict | None:
            normalized_path = self._normalize_path(finding.file_path)
            if normalized_path in unavailable or not self._reachability.supports_file(
                finding.file_path
            ):
                return None
            if graph.node_count() == 0:
                return _missing_graph_decision()
            request = ReachabilityTriageRequest(
                ...
                threat_model_context=format_threat_model_context(threat_model_context),
            )
            return runner.triage(request, graph)

        return classify


def _missing_graph_decision() -> dict[str, Any]:
    return {
        "status": "inconclusive",
        "reason": "No reachability graph was available for triage.",
        "evidence": [],
        "resolution_chain": [],
