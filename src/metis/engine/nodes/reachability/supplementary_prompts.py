...
_INTRA_SYS = _finding_prompt(
    """\
You are a vulnerability expert. Examine each function below for bugs WITHIN the function itself.
Look for:
...
_COMBINED_GRAPH_SYS = _finding_prompt(
    """\
Review the provided source code for the requested security analysis types only.
...
_CLASSIC_C_SINK_SYS = _finding_prompt(
    """\
You are analyzing selected functions that contain dangerous APIs.
...
_ERROR_UNWIND_SYS = _finding_prompt(
    """\
You are analyzing selected functions for error-unwind, cleanup, and rollback bugs.
Focus only on:
...
_COUNTER_SYMMETRY_SYS = _finding_prompt(
    """\
You are analyzing selected functions for counter, refcount, and accounting symmetry bugs.
...
_TARGET_PATH_ACCESS_SYS = _finding_prompt(
    """\
You are analyzing selected functions for path traversal and filesystem TOCTOU.
Target only:
