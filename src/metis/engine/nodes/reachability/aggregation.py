...
from metis.engine.nodes.reachability.finding_values import _safe_int
...
class ReviewResultAggregator:
    def __init__(
        self,
        config: EngineConfig,
        reachability_settings: dict[str, object] | None,
        *,
        final_adjudicator: Callable[..., object] | None = None,
    ...
    def aggregate(self, results: object, *, deduplicate: bool = True) -> object:
        ...
        return self._validate_review_groups(aggregated, aggregated["reviews"])

    def _deduplicate_review_groups(
        self,
        results: dict[str, Any],
        refs: list[tuple[int, int]],
        findings: list[VulnerabilityFinding],
    ) -> dict[str, Any]:
        if len(findings) < 2 or not callable(self._final_adjudicator):
            return results
        ...
        aggregated["reviews"] = _rewrite_review_groups(
            results["reviews"], item_replacements
        )
        logger.info(
    ...
    def _validate_review_groups(self, results, review_groups):
        candidates = []
        candidate_refs = []
        ...
        decisions = self._validator.validate_candidates(candidates)
        if not decisions:
            return results
    ...
    def _reachability_findings_from_review_groups(
        self,
        review_groups: list[Any],
    ) -> tuple[list[tuple[int, int]], list[VulnerabilityFinding]]:
        refs: list[tuple[int, int]] = []
        findings: list[VulnerabilityFinding] = []
        for group_index, item_index, item in _iter_review_items(review_groups):
