...
_MEMORY_BOUNDS_TYPES = frozenset(
    {
        ...
        "integer_overflow",
        "integer_overflow_in_allocation",
        "integer_overflow_allocation",
        "array_oob",
        "wrong_struct_field",
        "stale_length",
        "field_staleness_after_mutation",
        "field_staleness",
    }
)
...
def _compatible_dedupe_family(a, b):
    ...
    if types <= _AUTH_DEDUP_TYPES:
        ...
        if a["fn"] == b["fn"]:
            ...
            return bool(shared) or (
                a["line"] and b["line"] and abs(a["line"] - b["line"]) <= 5
            )
    ...
    if types <= _STRING_BOUNDS_TYPES:
        ...
        if (
            ...
            or (
                "title" in shared
                and bool(shared & {"terminate", "unterminate", "null"})
            )
            or len(shared) >= 4
        ):
            return True
...
def _same_root_cause(a, b):
    ...
    if (
        a["sink_line"]
        and a["sink_line"] == b["sink_line"]
        and a["sink_file"] == b["sink_file"]
    ):
        return True
    ...
    if types <= _FORMAT_STRING_TYPES:
        shared = (a["tokens"] & b["tokens"]) & _FORMAT_STRING_TOKENS
        if ("format" in shared or "printf" in shared or "vprintf" in shared) and len(
            shared
        ) >= 2:
            return True
    ...
    if (
        _token_overlap_score(a["tokens"], b["tokens"]) >= 0.62
        and len(a["tokens"] & b["tokens"]) >= 4
    ):
        return True
    ...
    if types <= _CALLBACK_LIFECYCLE_TYPES:
        shared = (a["tokens"] & b["tokens"]) & _CALLBACK_LIFECYCLE_STRONG_TOKENS
        if shared and (
            not a["line"] or not b["line"] or abs(a["line"] - b["line"]) <= 25
        ):
            return True
...
def _collapse_by_root_cause(findings):
    ...
    for idx, info in enumerate(infos):
        ...
        if (
            info["vtype"] in _CALLBACK_LIFECYCLE_TYPES
            and info["tokens"] & _CALLBACK_LIFECYCLE_STRONG_TOKENS
        ):
            by_callback_file[info["file"]].append(idx)
        if info["vtype"] in _AUTH_DEDUP_TYPES:
            by_auth_location[(info["file"], info["fn"])].append(idx)
            if (
                info["vtype"] in _AUTH_HELPER_DEDUP_TYPES
                and info["tokens"] & _AUTH_HELPER_TOKENS
            ):
                by_auth_helper["auth_level"].append(idx)
        if (
            info["vtype"] in _FORMAT_STRING_TYPES
            and info["tokens"] & _FORMAT_STRING_TOKENS
        ):
            by_format_signal["format"].append(idx)
        if (
            info["vtype"] in _STRING_BOUNDS_TYPES
            and info["tokens"] & _STRING_BOUNDS_TOKENS
        ):
    ...
    for key, group in groups.items():
        ...
        for f in group:
            ...
            for cluster in clusters:
                if _description_overlap(
                    _finding_text(f), _finding_text(cluster[0]), threshold=0.55
                ):
                    cluster.append(f)
                    merged = True
                    break
