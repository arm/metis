...
def test_c_family_analyzer_fallback_targets_filter_macro_noise():
    ...
    targets = analyzer._compute_fallback_targets_from_unresolved(
        unresolved_hops=[
            ...
            "MACRO_SEMANTICS_UNRESOLVED:KAI_ASSUME",
        ],
        preferred_symbols=["__aarch64__", "helper", "KAI_ASSUME"],
        limit=8,
    )
...
def test_c_family_analyzer_resolves_macro_chain(tmp_path):
    src = tmp_path / "proj"
    src.mkdir()
    hdr = src / "kai_common.h"
    hdr.write_text(
        "#define KAI_ASSERT_MSG(cond,msg) do{}while(0)\n"
        "#define KAI_ASSERT(cond) KAI_ASSERT_MSG(cond,#cond)\n"
        "#define KAI_ASSUME KAI_ASSERT\n",
        encoding="utf-8",
    )
    cfile = src / "main.c"
    cfile.write_text('#include "kai_common.h"\nint x;\n', encoding="utf-8")
    ...
    sections, citations, resolution, unresolved = analyzer._analyze_macro_semantics(
        symbols=["KAI_ASSUME"],
        include_files=["main.c", "kai_common.h"],
        codebase_path=str(src),
    )
    ...
    assert citations
    assert resolution
    assert not any("MACRO_SEMANTICS_UNRESOLVED:KAI_ASSUME" == u for u in unresolved)
...
def test_c_family_analyzer_prefers_asm_impl_when_decl_and_asm_exist():
    ...
    hit = analyzer._choose_best_symbol_hit(
        [
            _Hit("kai_commit_za", "kai/kai_common.h", 225, "declaration"),
            _Hit("kai_commit_za", "kai/kai_common_sme_asm.S", 48, "asm_label"),
        ]
    )
